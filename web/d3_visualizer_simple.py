"""
Simplified D3.js Graph Visualizer for Testing

This module provides D3.js-based graph visualization capabilities without
streamlit dependencies for testing purposes.
"""
...
"""
    ...
    def visualize_expression(self, expression: str, width: int = 800, height: int = 600) -> str:
        ...
        try:
            # Try to import and use the real graph conversion
            try:
    ...
    def create_standalone_visualization(self, expression: str, output_path: str = None) -> str:
...
def render_d3_graph(expression: str, width: int = 800, height: int = 600):
    """Convenience function to render D3.js graph."""
    if STREAMLIT_AVAILABLE:
        visualizer = D3GraphVisualizer()
        html_content = visualizer.visualize_expression(expression, width, height)
        components.html(html_content, height=height + 100, scrolling=False)
    else:
        print(f"D3.js visualization for expression: {expression}")
        print("Streamlit not available - use create_standalone_visualization() instead")
